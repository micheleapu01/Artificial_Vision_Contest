...
def load_mot_file(file_path: str) -> pd.DataFrame:
    ...
    if df.shape[1] < 7:
        raise ValueError(f"File MOT non valido (<7 colonne): {file_path}")
    ...
    for a, sa in summ.items():
        ...
        for gap in range(1, max_gap + 1):
            ...
            for b in starts_by_frame.get(t_b, []):
                ...
                thr = dist_base + dist_per_gap * gap
                if dpos > thr:
                    continue

                cost = dpos + size_weight * d_size
...
def main():
    ...
    p.add_argument("--border-frac", type=float, default=0.06)
    p.add_argument("--allow-border", action="store_true")

    p.add_argument("--dist-base", type=float, default=0.9)
    p.add_argument("--dist-per-gap", type=float, default=0.12)
    p.add_argument("--size-log-gate", type=float, default=0.55)
    p.add_argument("--size-weight", type=float, default=0.15)
    ...
    for f in files:
        ...
        print(f" {base}: IDs {n0} -> {n1} | merges={len(links)}")
